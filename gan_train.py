...
f.write('\n'+"d:"+config().d_config+"+"+"g:"+config().g_config+'\n\n')
f.close()
print("write"+'\n'+"d:"+config().d_config+"+"+"g:"+config().g_config+'\n\n'+"to "+"plotdata/"+config().task.dir+".txt")
dis.test_link(test_data, n_ent, filt_heads, filt_tails,epo=0)#epoch=0
...
for epoch in range(n_epoch):
    ...
    for s, r, t, ss, rs, ts in batch_by_num(n_batch, src, rel, dst, src_cand, rel_cand, dst_cand, n_sample=n_train):
        ...
        src_smpl, dst_smpl, rel_smpl = next(gen_step)
        losses, rewards = dis.dis_step(s, r, t, src_smpl.squeeze(), dst_smpl.squeeze(), rel_smpl.squeeze())
    ...
    if (epoch + 1) % config().adv.epoch_per_test == 0:
        ...
        perf = dis.test_link(valid_data, n_ent, filt_heads, filt_tails,epo=epoch+1)
        print("in process... "+"d:"+config().d_config+"+"+"g:"+config().g_config+"  "+config().task.dir)
        if perf > best_perf:
            best_perf = perf
