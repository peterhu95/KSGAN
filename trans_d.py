...
class TransDModule(BaseModule):
    ...
    def dist(self, src, rel, dst,good=True):
        if not good:
            #print()
            #print(self.forward(src, rel, dst))
            #print()
            ret, _=t.min(self.forward(src, rel, dst),1,keepdim=True)
            return ret 
        else:
            return self.forward(src, rel, dst)

    def score(self, src, rel, dst,good=True):  ##
        #if not good:
            #print()
            #print(self.forward(src, rel, dst))
            #print()
